...
class MultiPredictionEnsemble(PolicyPredictionEnsemble):
    """Interface for any policy prediction ensemble that allows multiple predictions from policies.
    This allows for more efficiency by allowing LLM based predictions to predict all
    next actions until the next user query should be awaited in just one query.

    For more information, see PolicyPredictionEnsemble.
    """
    ...
    def _best_policy_prediction(
        ...
        return MultiPredictionEnsemble._pick_best_policy(predictions)
